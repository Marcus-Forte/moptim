...
struct DoubledStepPlusOperator {
  static void plus(const double* x, const double* delta, double* out,
                   size_t dimensions) {
...
struct OffsetLineModel {
  ...
  void residual(const double* /*x*/, const double* /*input*/, const double* obs,
                double* res) const {
    res[0] = obs[0] - state_;
  }
...
TEST(UpdateRule, NumericalForwardEulerUsesCustomPlus) {
  ...
  moptim::NumericalCostForwardEuler<OffsetLineModel, double,
                                    DoubledStepPlusOperator>
      cost(
      input, observation, 1, 1, 1, 1);
