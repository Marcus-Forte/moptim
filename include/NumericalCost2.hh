...
class NumericalCost2 : public ICost {
 public:
  NumericalCost2(const NumericalCost2&) = delete;

  NumericalCost2(const double* input, const double* observations, size_t input_size, size_t output_dim,
                 const std::shared_ptr<IModel>& model,
                 DifferentiationMethod method = DifferentiationMethod::BACKWARD_EULER);

  ~NumericalCost2() = default;
