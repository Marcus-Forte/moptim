...
static inline bool isDeltaSmall(const double* data, size_t size) {
  Eigen::Map<const Eigen::Vector<double, Eigen::Dynamic>> vec_map(data, size);
  const auto epsilon = vec_map.array().abs().maxCoeff();
  return epsilon < sqrt(std::numeric_limits<double>::epsilon());
}

static inline bool isDeltaSmall(const float* data, size_t size) {
  Eigen::Map<const Eigen::Vector<float, Eigen::Dynamic>> vec_map(data, size);
  const auto epsilon = vec_map.array().abs().maxCoeff();
  return epsilon < 1e-5;
}
