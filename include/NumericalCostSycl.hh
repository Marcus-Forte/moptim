...
class NumericalCostSycl : public ICost<T> {
 public:
  ...
  NumericalCostSycl(const std::shared_ptr<ILog>& logger, const sycl::queue& queue, std::span<const T> input,
    ...
    if (!queue_.get_device().is_cpu()) {
      ...
      observations_sycl_ = std::span<T>(sycl::malloc_device<T>(observation_dim_ * num_elements, queue_),
                                        observation_dim_ * num_elements);

      queue_.copy<T>(input.data(), input_sycl_.data(), observation_dim_ * num_elements);
      queue_.copy<T>(observations.data(), observations_sycl_.data(), observation_dim_ * num_elements);

    } else {  // No need to copy data if it already lies in a CPU (host) device
      input_sycl_ = std::span<T>(const_cast<T*>(input.data()), input.size());
      observations_sycl_ = std::span<T>(const_cast<T*>(observations.data()), observations.size());
    }
  ...
  ~NumericalCostSycl() override {
    if (!queue_.get_device().is_cpu()) {
      sycl::free(input_sycl_.data(), queue_);
      sycl::free(observations_sycl_.data(), queue_);
    }

    sycl::free(cost_reduction_.data(), queue_);
    sycl::free(jacobian_data_.data(), queue_);
    sycl::free(residual_data_.data(), queue_);
    sycl::free(residual_plus_data_.data(), queue_);
  }
  ...
  void computeLinearSystem(const T* x, T* JTJ, T* JTb, T& cost) override {
    ...
    auto models_sycl_plus = std::span<Model>(sycl::malloc_device<Model>(param_dim_, queue_), param_dim_);
    ...
    const auto mirror_jtj = queue_.submit([&](sycl::handler& cgh) {
      ...
      auto* JTJ_device_capture = JTJ_device.data();
      const auto param_dim_capture = param_dim_;
 ...
 private:
  ...
  std::span<T> input_sycl_;
  std::span<T> observations_sycl_;
  std::span<T> cost_reduction_;
  std::span<T> jacobian_data_;
  std::span<T> residual_data_;
  std::span<T> residual_plus_data_;
  sycl::queue queue_;
};
